...
def file_object_compare(in_fil_a, in_fil_b, numbyte):
    ...
    pos = 0
    while numbyte > pos:
        ca = in_fil_a.read(1024)
        if ca == '':
            break
        cb = in_fil_b.read(1024)
        if cb == '':
            break
        if ca != cb:
...
def file_compare(in_filename_a, in_filename_b):
    ...
    numbyte = getsize(in_filename_a)
    result = file_object_compare(in_fil_a, in_fil_b, numbyte)
    in_fil_a.close()
    in_fil_b.close()
    return result
